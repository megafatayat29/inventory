...
function PhotoPreview({ path }: { path: string | null }) {
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer">
      <img
        src={url}
        alt="Foto penitipan barang bekas"
...
export default function UsedGoodsDetailModal({
  usedGoods,
  onClose,
}: UsedGoodsDetailModalProps) {
  if (!usedGoods) return null
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 py-8 overflow-y-auto">
      <div className="w-full max-w-2xl bg-white rounded-xl shadow-lg overflow-hidden border-t-8 border-blue-800 my-auto">
        <div className="bg-[#1e3a8a] px-6 py-4 text-white flex items-center justify-between">
          <h2 className="text-lg font-bold">Detail Barang Bekas</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-6 space-y-8">
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Field label="Nama" value={usedGoods.depositor_name} />
              <Field label="NIPP" value={usedGoods.nipp} />
              <Field label="Jabatan" value={usedGoods.jabatan} />
              <Field label="Unit Kerja" value={usedGoods.unit_kerja} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Field label="Unit Penghasil" value={usedGoods.producing_unit} />
              <Field label="Tanggal Masuk" value={formatDate(usedGoods.entry_date)} />
              <Field label="Dicatat Pada" value={formatDate(usedGoods.created_at)} />
              <div className="md:col-span-2">
                <Field label="Alasan Titip" value={usedGoods.reason} />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <PhotoPreview path={usedGoods.photo_path} />
          </div>
        </div>
